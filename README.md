...
2. **FlashAttention-3 是隐性硬依赖**：DiT 在 `--batch_cfg`（batch>1 走 packed 注意力）或 context parallel 下强制要求 FA3（`flash_attn_interface`），但上游 requirements 没带它，干净环境下上游单卡脚本默认参数必报 `flash_attn_interface.flash_attn_varlen_func is required`。`setup_env.sh` 会在 Hopper 上源码编译安装（~20–40 分钟，并行度 `FA3_MAX_JOBS` 默认 16）；若镜像的 nvcc 与 torch 的 CUDA 大版本不匹配（torch cpp_extension 会硬报错），脚本自动从 NVIDIA apt 源安装匹配的 cuda-toolkit（~4GB）。编译失败或 `SKIP_FA3=1` 跳过时的绕过：运行加 **`BATCH_CFG=0`**——B=1 走 SDPA 路径，完全不需要 FA3，代价是 CFG 两分支串行、稍慢。约定：**不改上游模型代码**，依赖问题一律在部署层解决。
