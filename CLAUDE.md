...
- **别想复用 verl-env-cache 里的 flash-attn**（已调研，2026-07-15）：那是 FA2（`flash_attn==2.8.3`，导入路径 `flash_attn.*`，且绑定其 conda/torch ABI），而 lingbot 需要 FA3 的顶层 `flash_attn_interface` 模块——不同的包。官方（PyPI/GitHub releases）也没有 FA3 预编译 wheel，源码编译 + 自建缓存是唯一路径。FA3 wheel 是 sm80/sm90a/sm100a 多架构全家桶，A100/H100/H200/B200 通用。
